...
const MediaItem = ({ item, className, onClick }: { item: MediaItemType, className?: string, onClick?: () => void }) => {
    ...
    useEffect(() => {
        ...
        if (currentVideo) {
            observer.observe(currentVideo); // Start observing the video element
        }

        return () => {
            if (currentVideo) {
                observer.unobserve(currentVideo); // Clean up observer when component unmounts
            }
        };
    }, []);
    ...
    useEffect(() => {
        ...
        const handleVideoPlay = async () => {
            if (!currentVideo || !isInView || !mounted) return; // Don't play if video is not in view or component is unmounted

            try {
                if (currentVideo.readyState >= 3) {
                    setIsBuffering(false);
                    await currentVideo.play(); // Play the video if it's ready
                } else {
                    setIsBuffering(true);
                    await new Promise((resolve) => {
                        if (currentVideo) {
                            currentVideo.oncanplay = resolve; // Wait until the video can start playing
                        }
                    });
                    if (mounted) {
                        setIsBuffering(false);
...
const GalleryModal = ({ selectedItem, isOpen, onClose, setSelectedItem, mediaItems }: GalleryModalProps) => {
    ...
    return (
        <>
            ...
            >
                ...
                <motion.button
                    className="absolute top-2 sm:top-2.5 md:top-3 right-2 sm:right-2.5 md:right-3 
                              p-2 rounded-full bg-white/20 text-white hover:bg-white/30 
            ...
            >
                <motion.div
                    className="relative rounded-xl bg-black/40 backdrop-blur-xl 
                ...
                >
                    <div className="flex items-center -space-x-2 px-3 py-2">
                        {mediaItems.map((item, index) => (
                            <motion.div
                                ...
                                className={`
                                    ...
                                    ${selectedItem.id === item.id
                                        ? 'ring-2 ring-rose-400 shadow-lg scale-110'
                            ...
                            >
                                <MediaItem item={item} className="w-full h-full animate-none" onClick={() => setSelectedItem(item)} />
                                <div className="absolute inset-0 bg-gradient-to-b from-transparent via-white/5 to-white/20" />
                                {selectedItem.id === item.id && (
                                    <motion.div
                                        layoutId="activeGlow"
                                        className="absolute -inset-2 bg-rose-400/20 blur-xl"
                                        initial={{ opacity: 0 }}
                                        animate={{ opacity: 1 }}
...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({ mediaItems, title, description }) => {
    ...
    // Keep items state in sync if prop changes
    useEffect(() => {
        setItems(mediaItems);
    }, [mediaItems]);
                    ...
                    >
                        {items.map((item, index) => (
                            <motion.div
                                ...
                                className={`relative overflow-hidden rounded-2xl cursor-move border border-white/5 bg-neutral-900/40 ${item.span}`}
                            ...
                            >
                                ...
                                >
                                    <div className="absolute inset-0 flex flex-col justify-end p-3 sm:p-4">
                                        ...
                                        <h3 className="relative text-white text-xs sm:text-sm md:text-base font-semibold line-clamp-1">
                                            {item.title}
                                        </h3>
                                        {item.desc && (
                                            <p className="relative text-white/70 text-[10px] sm:text-xs mt-0.5 line-clamp-2">
                                                {item.desc}
                                            </p>
                                        )}
                                    </div>
